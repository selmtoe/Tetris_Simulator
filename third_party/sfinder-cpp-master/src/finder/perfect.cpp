...
namespace finder {
    ...
    void PerfectFinder<core::srs::MoveGenerator>::accept(const Configure &configure, const Record &record) {
        ...
        // This is an existence query in the simulator, not the command-line
        // tool's best-score search.  A first valid PC is enough and lets the
        // recursive search return immediately.
        best = Record(record);
        foundAny = true;
    }
    ...
    void PerfectFinder<core::srs::MoveGenerator>::move(
        ...
        for (const auto &move : moves) {
            ...
            if (nextLeftLine == 0) {
                // The original C++ port accepted "the requested number of
                // lines cleared" even if blocks remained above them.  The
                // in-game guide must only report a true perfect clear.
